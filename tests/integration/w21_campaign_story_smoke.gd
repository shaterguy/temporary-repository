...
func _run_story_smoke() -> void:
    ...
    var settlement: Dictionary = campaign.settle_current("success", {}, {})
    if not bool(settlement.get("ok", false)) or not campaign.world.has_pending_story_event():
    ...
    if not _ending_reload_ok:
        ...
        if not bool(shell.call("select_world_choice", 0)):
            ...
            _next_expedition_modifier_ok = (
                ...
                and str(context.get("support_id", "")) != ""
