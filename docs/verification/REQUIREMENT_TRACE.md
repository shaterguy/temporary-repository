# Requirement trace — W11 checkpoint
...
| AC-08 | N05 disclosed enemy doctrine | PARTIAL | W11 adds deterministic doctrine selection from a prior observation summary, low-sample neutrality, repeated-failure relief, departure-readable disclosure data, combat prewarning, a ≤30% response-formation cap, actual W05 composition/placement changes, tamper-rejecting plan snapshots, and route/circuit/phase counterplay that is character-independent. W12 still must persist the observation summary/selected plan through the real expedition→hub→next-region loop, and production UI/presentation/balance remain later. |
| AC-09 | N06 persistent world graph | NOT_IMPLEMENTED | W12. |
| AC-10 | N07 dual-phase battlefield | PARTIAL | W08 adds same-coordinate material/shadow blockers and cover, destination safety preview, no-cost rejection, cooldown, pause/cancel gating, phase-aware player collision and enemy targeting, actual W07 circuit phase switching, explicit cross-phase boss warning before damage, and phase snapshot/restore. W09 supplies the current phase to causal weapon transforms. W10 resolves replay movement against the current phase geometry and suppresses recorded shots whose recorded phase differs from the current battlefield. W11 doctrine counterplay explicitly retains phase switching as a system-level response. Production maps, final art and later world integration remain. |
| AC-11 | Campaign/hub/save/retry/post-final loop | PARTIAL | Versioned checksum save envelope + primary/backup primitive, W06 route snapshot/restore, W07 active-circuit snapshot/restore, W08 phase snapshot/restore, W09 weapon snapshot/migration, W10 save-ready tactical-echo selection/consumed-circuit snapshot, and a W11 tamper-rejecting doctrine-plan snapshot primitive exist. W10 intentionally cancels active replay and discards in-progress capture on restore to prevent duplicate side effects. World/campaign/settlement persistence, actual cross-expedition record transport and doctrine observation-summary persistence remain W12. |
| AC-12 | Distinct content manifest + no release placeholders | PARTIAL | Manifest holds the full planned counts and release gate; implemented production content remains 0. W04 training geometry, W05 swarm preview, W06 ark geometry, W07 circuit lines, W08 phase blockers and W10 echo rendering primitive are development instrumentation. W09 representative weapon definitions and W11 doctrine formations are gameplay prototypes and do not count as final production content. |
| AC-13 | Actual art/animation/audio/readability review | NOT_IMPLEMENTED | W04∼W11 procedural preview/instrumentation is not presentation acceptance evidence. |
...
- W12 remains responsible for persisting the observation summary/selected doctrine through the real expedition→hub→next-region lifecycle and for binding departure disclosure to production campaign UI.
